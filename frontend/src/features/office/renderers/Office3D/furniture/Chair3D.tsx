...
interface Chair3DProps { position: [number, number, number]; rotationY?: number; isDark?: boolean }
const angles = [0, 72, 144, 216, 288].map(a => a * Math.PI / 180)
export const Chair3D: React.FC<Chair3DProps> = ({ position, rotationY = 0, isDark = true }) => {
  const p = getOfficePalette(isDark)
  return <group position={position} rotation={[0, rotationY, 0]}>
    <FabricBox position={[0, .52, 0]} scale={[.62, .075, .56]} color={p.officeChairCushion} />
    <SatinBox position={[0, .51, .27]} scale={[.58, .04, .06]} color={p.officeChairFrame} />
    <FabricBox position={[0, .91, .24]} rotation={[-.1, 0, 0]} scale={[.54, .70, .055]} color={p.officeChairCushion} />
    <SatinBox position={[0, .82, .264]} rotation={[-.1, 0, 0]} scale={[.34, .14, .022]} color={p.officeMetal} />
    <FabricBox position={[0, 1.295, .21]} rotation={[-.1, 0, 0]} scale={[.30, .18, .055]} color={p.officeChairCushion} />
