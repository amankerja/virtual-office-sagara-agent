...
class SeedPreviewer:
    ...
    def _load_current_profiles(self) -> dict[str, Any]:
        ...
        # Attempt to load from project root core.registry.profile
        root = self.validator.project_root
        if root:
            try:
                from core.registry.profile import ProfileRegistry
                reg = ProfileRegistry.load(root)
    ...
    def generate_preview(self) -> PreviewReport:
        ...
        for p_data in sorted(profiles_raw, key=lambda x: x["id"]):
            ...
            if not cur_p:
                ...
                cur_model = getattr(cur_p, "model_tier", None) or (cur_p.get("model_tier") if isinstance(cur_p, dict) else None)
                model_diff = cur_model != m_pol.get("primary_tier")

                if role_diff or skills_diff or model_diff:
                    action = "WOULD_UPDATE"
